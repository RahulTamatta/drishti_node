    ...
    return Scaffold(
      ...
      body: BlocBuilder<ProfileDetailsBloc, ProfileDetailsState>(
        builder: (context, state) {
            ...
            if (_selectedOption.value == YesNoOption.none) {
              ...
              if (userDetails?.role == "teacher" &&
                ...
                return TeacherProfileScreen(
                    userDetails:
                        userDetails!); // Replace with your teacher screen
              } else if (userDetails?.role == "user" &&
                      userDetails?.email != null ||
                  userDetails?.role == "teacher") {
                return UserProfileScreen(
                    userDetails: userDetails!); // Replace with your user screen
              }
            }
          ...
          return const ProfileShimmerEffect(); // Replace with your shimmer effect
        },
      ),
    );
  }
  ...
  Future<void> profileApi(BuildContext context1) async {
    ...
    try {
      ...
      final profileImageFile = await MultipartFile.fromFile(
        ...
        contentType: MediaType.parse(lookupMimeType(profileImage!.path)!),
      );
      formData.files.add(MapEntry('profileImage', profileImageFile));

      // Trim and validate fields before adding
      ...
      // Handle teacher role if selected
      if (_selectedOption.value == YesNoOption.yes) {
        if (_imageFile == null) {
          showToast(
        ...
        formData.fields.add(MapEntry('teacherId', _teacherIdController.text));
